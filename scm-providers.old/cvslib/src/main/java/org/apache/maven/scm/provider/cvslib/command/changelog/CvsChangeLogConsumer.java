 ...
 * @version $Id$
 */
public class CvsChangeLogConsumer implements ChangeLogConsumer
{
    /**
    ...
    private void addEntry(ChangeLogEntry entry, ChangeLogFile file)
    {
        ...
        String key =
            ENTRY_KEY_TIMESTAMP_FORMAT.format(entry.getDate())
                + entry.getAuthor()
                + entry.getComment();
    ...
    private void processGetFile(String line)
    {
        if (line.startsWith(START_FILE))
        {
            ...
            setCurrentFile(
                new ChangeLogFile(
                    line.substring(START_FILE.length(), line.length())));
